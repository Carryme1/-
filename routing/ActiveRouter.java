...
public abstract class ActiveRouter extends MessageRouter {
	...
	public Message tryAllMessages(Connection con, List<Message> messages) {

		for (Message m : messages) {
			//send a msg for a con
			//delivery ferry. exp:area 除去某区域的消息，其他不参与转发
//			if(list.contains(getHost().getAddress())||getHost().getAddress()==85) {
//				int i = m.getTo().getAreaId();
//				int b = con.getOtherNode(getHost()).getAddress();
//				int areaid = con.getOtherNode(getHost()).getAreaId();
//				if (i != areaid &&getHost().getAddress()==85)
//					continue;
//				//collect fixed. exp:area 只转发区域间的消息,当前节点是摆渡节点且链接的另一端是摆渡节点且消息包的区域id等于当前区域中心节点的区域id
//				if (b == 85 && list.contains(getHost().getAddress())&& i==areaid) {
//					continue;
//				}
//			}
	...
	protected Connection tryMessagesToConnections(List<Message> messages,
		...
		for (int i=0, n=connections.size(); i<n; i++) {
			...
			//send all msg for a con,if sucess return msg
